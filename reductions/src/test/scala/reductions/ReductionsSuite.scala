...
class ReductionsSuite extends munit.FunSuite:
  ...
  test("should return maximum angle for given part") {
    val from = 0
    val until = 2
    val maxFromUntil = upsweepSequential(Array[Float](0f, 1f, 8f, 9f), from, until)
    assertEquals(maxFromUntil, 1f)
  }

  test("should return maximum angle for given part 2.0") {
    val from = 0
    val until = 3
    val maxFromUntil = upsweepSequential(Array[Float](0f, 1f, 8f, 9f), from, until)
    assertEquals(maxFromUntil, 4f)
  }
